...
export function TraficCreatif({
  ...
  const batches = clients.flatMap((c) => c.batches);
  ...
  return (
    <div>
      ...
      <p className="mb-6 font-body text-sm text-asight-dark/50">
        {batches.length} batch{batches.length !== 1 ? 's' : ''} au total, tous clients
        confondus. Glissez une carte pour changer son statut.
      </p>
            ...
            >
              ...
              {columnBatches.map((batch) => (
                <div
                  ...
                  className={`cursor-grab rounded-2xl border border-asight-lavande bg-white p-4 shadow-card transition-opacity active:cursor-grabbing ${
                ...
                >
                  ...
                  <p className="mb-1 font-body text-sm font-bold text-asight-dark">
                    {batch.displayName}
                  </p>
