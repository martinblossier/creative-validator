...
export function BatchCard({ batch }: { batch: BatchOverview }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-asight-lavande bg-white shadow-card">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full flex-wrap items-center justify-between gap-3 px-5 py-4 text-left transition-colors hover:bg-asight-lavande/30"
      >
        <div className="flex flex-wrap items-center gap-3">
          <span className="rounded-full bg-asight-violet px-3 py-1 font-body text-sm font-bold text-white">
            {batch.batchLabel}
          </span>
        ...
        <span className="font-heading text-lg text-asight-dark/40">
          {open ? '−' : '+'}
        </span>
      </button>
